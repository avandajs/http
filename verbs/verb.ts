...
export default function (middlewares: Middleware[],method: string){
    ...
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = async function (...args: any) {
            ...
            if (request?.method?.toLowerCase() !== method && !request.isWatcher) {//invalid request
                return function (...args: any){
                    return response.error(`Bad request type, ${request?.method}`,500)
                }
            }else if (request?.method?.toLowerCase() == 'options' && !request.isWatcher){
                return function (...args: any){
                    return response.success('Preflight',200)
                }
            }
            //check for middlewares validity
            let middlewareCheck = await validate(middlewares,response,request,model)
